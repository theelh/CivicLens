...
export default function Show() {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: 'My Reports',   href: '/staff/reports/all' },
        { title: report.title ?? `Report ${report.id}`, href: `/staff/reports/${report.id}` },
    ];
    ...
    const handleAnalyze = () => {
        setAnalyzing(true);
        router.post(`/staff/reports/${report.id}/analy`, {}, {
            preserveScroll: true,
    ...
    const handleMarkResolved = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.post(`/staff/reports/${report.id}/resolve`, {}, { preserveScroll: true });
            }
        });
    };
    const handleMarkClosed = () => {
        Swal.fire({
            ...
            text: 'This will update the report status to closed.',
            icon: 'question',
            showCancelButton: true,
            confirmButtonColor: '#10b981',
            cancelButtonColor: '#6b7280',
            ...
            if (result.isConfirmed) {
                router.post(`/staff/reports/${report.id}/closed`, {}, { preserveScroll: true });
            }
        });
    };
    ...
    const handleDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(`/staff/reports/${report.id}/destroy`, {
                    onSuccess: () => {
                        Swal.fire({
                            icon: 'success',
                            title: 'Deleted!',
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
                ...
                <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        {statusMeta.label !== 'closed' && (
                        <button
                            onClick={handleMarkClosed}
                            className="flex items-center gap-1.5 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100 dark:border-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-300"
                        >
                            <EyeClosedIcon/> Mark Closed
                        </button>
                        )}
